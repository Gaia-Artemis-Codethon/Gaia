...
class ChatSupabase {
  ...
  Future<void> deleteChatRoomById(Guid id) async {
    try {
      await SupabaseService().deleteData("ChatRooms", id.value);
    } catch (e) {
      print('Error en clase chat_supabase al borrar datos: $e');
    }
  }
  ...
  Future<bool> insertMessage(
      Guid postId, Guid clientId, Guid sellerId, types.Message messages) async {
    try {
  ...
  Future<String> checkNewChat(Guid postId, Guid clientId, Guid sellerId) async {
    try {
      ...
      print(
          'Error en clase chat_supabase al comprobar si es primer mensaje: $e');
      return "";
    }
  }
}
